...
var forwardGeocodeNative = function(address, callback) {
	...
	xhr.onload = function() {
		...
		if (json.status != 'OK') {
	    	Alloy.Globals.core.showErrorDialog('Unable to geocode the address');
	    	return;
	    }
	...
	xhr.onerror = function(e) {
		...
		Alloy.Globals.core.showErrorDialog(ERROR_MESSAGE);
	};
	xhr.send();
};
...
var forwardGeocodeWeb = function(address, callback) {
	...
	if (geocoder) {
		...
		Alloy.Globals.core.showErrorDialog('Google Maps Geocoder not supported');
	}
};

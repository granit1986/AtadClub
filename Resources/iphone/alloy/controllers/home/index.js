...
function Controller() {
    ...
    function deviceTokenError(e) {
        Ti.API.error(e);
        indicator.closeIndicator();
        if ("Simulator" == Ti.Platform.model || "google_sdk" === Ti.Platform.model) {
    ...
    function notifyReceive(data) {
        ...
        if (Alloy.Globals.core.applicationPaused && data.newChatMessage && inBackground) {
            ...
            message[1] != Alloy.Globals.chat.openChatId && (view = Alloy.createController("account/answers/answer", {
                newChat: false,
                id: message[1],
                toUser: message[2],
                UserName: message[4]
            ...
            var message = JSON.parse(data.newChatMessage);
            if (!Alloy.Globals.chat.connected) if (message[1] != Alloy.Globals.chat.openChatId) Alloy.Globals.chat.notify(L("chat"), L("new_message"), message, function(d) {
                ...
                if (Alloy.Globals.tabGroup.activeTab) Alloy.Globals.tabGroup.activeTab.open(view); else {
            ...
            var view = false;
            Alloy.Globals.core.applicationPaused ? view = offerData.FromUserId != Alloy.Globals.profile.id ? Alloy.createController("account/offers/offer", {
                offerId: offerData.Id
            }).getView() : Alloy.createController("account/offers/myOffer", {
                offerId: offerData.Id
            }).getView() : Alloy.Globals.chat.notify(L("offer"), data.alert, data, function(d) {
                view = d.FromUserId != Alloy.Globals.profile.id ? Alloy.createController("account/offers/offer", {
                    offerId: offerData.Id
                }).getView() : Alloy.createController("account/offers/myOffer", {
                    offerId: offerData.Id
                }).getView();
        ...
        } else data.newDeals ? Alloy.Globals.chat.notify(L("new_deals"), data.alert, data) : data.deals && Alloy.Globals.chat.notify(L("deals"), data.alert, data);
        if (view) if (Alloy.Globals.tabGroup.activeTab) Alloy.Globals.tabGroup.activeTab.open(view); else {
            Alloy.Globals.tabGroup.setActiveTab(Alloy.CFG.tabAccount);
            Alloy.Globals.tabGroup.activeTab.open(view);
        }
    }
    ...
    function goToNotifications() {
        indicator.closeIndicator();
        Alloy.createController("notifications/index").getView();
    ...
    function NewFetch() {
        ...
        indicator.openIndicator();
        subcats = [];
        if (!Alloy.Globals.notify.subCategories || Alloy.Globals.notify.subCategories.length < 3) return;
    ...
    var getHomeDeals = function() {
        ...
        geo.checkLocation(function() {
            ...
            NewFetch();
            geo.reverseGeocoding(geo.location.latitude, geo.location.longitude, function(e) {
                e && e.error ? Alloy.Globals.core.showErrorDialog(e.message ? L(e.message) : L(e.error)) : e && e.response && (e.response.results && e.response.results[0] && e.response.results[0].formatted_address ? $.address.value = e.response.results[0].formatted_address : e.response.resourceSets && e.response.resourceSets[0] && e.response.resourceSets[0].resources && e.response.resourceSets[0].resources[0] && e.response.resourceSets[0].resources[0].address && e.response.resourceSets[0].resources[0].address && e.response.resourceSets[0].resources[0].address.formattedAddress && ($.address.value = e.response.resourceSets[0].resources[0].address.formattedAddress));
            });
        });
    };
    var subcats = [];

...
Titanium.App.serverDomain = DEBUG ? "192.168.0.188" : "atadclub.slavin.biz";
...
Alloy.Globals.chat = {
    ...
    openConnect: function() {
        ...
        self.source.addEventListener("message", function(e) {
            self.notSendedMessage && (self.notSendedMessage = false);
            console.log(e.data);
            ...
            if (self.openChatId == result.ChatId) {
                ...
                self.messagesWindow.data[0] && self.messagesWindow.scrollToIndex(self.messagesWindow.data[0].rows.length - 1);
                var counts = Alloy.createModel("counts", {
                    id: result.MessageId
                });
                result.FromUserId !== Alloy.Globals.profile.id && counts.save({}, {});
            } else self.notify(result.FromUser, result.Text, result, function(data) {
                var view = Alloy.createController("account/answers/answer", {
                    newChat: false,
                    id: data.ChatId,
        ...
        self.source.addEventListener("open", function() {
            console.log("open!");
            self.connected = true;
            self.connecting = false;
            self.indicator.closeIndicator();
            self.notSendedMessage && self.source.send(self.notSendedMessage);
        });
        self.source.addEventListener("close", function() {
            console.log("close!");

var geo = {
    ...
    reverseGeocoding: function(lat, lng, callback, useBing) {
        ...
        this.xhr.onload = function() {
            ...
            responseJSON && responseJSON.status && responseJSON.status != self.responseStatuses.OK ? responseJSON.status === self.elementStatuses.ZERO_RESULTS ? geo.reverseGeocoding(lat, lng, callback, true) : callback({
                error: responseJSON.status
            }) : self.callback({
                error: false,
                response: responseJSON

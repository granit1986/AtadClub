...
function binb2str(bin) {
    ...
    for (var i = 0; i < 32 * bin.length; i += chrsz) str += String.fromCharCode(bin[i >> 5] >>> 32 - chrsz - i % 32 & mask);
    return str;
}
...
function binb2b64(binarray) {
    ...
    for (var i = 0; i < 4 * binarray.length; i += 3) {
        ...
        for (var j = 0; 4 > j; j++) str += 8 * i + 6 * j > 32 * binarray.length ? b64pad : tab.charAt(triplet >> 6 * (3 - j) & 63);
    }
    return str;
}
...
null == OAuth && (OAuth = {}), OAuth.setProperties = function(into, from) {
    ...
    setParameter: function(message, name, value) {
        ...
        if (parameters instanceof Array) {
            for (var p = 0; p < parameters.length; ++p) parameters[p][0] == name && (void 0 === value ? parameters.splice(p, 1) : (parameters[p][1] = value, 
            value = void 0));
...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
    function showLoading() {
        if (loading) return;
        loading = !0, loadingView && (loadingView.value = 0), estimateID = firstLoad ? "tokenRequest" : "pageLoad", 
    ...
    this.showLoadingUI = function() {
        ...
        var isIOS7 = true && parseInt(Ti.Platform.version, 10) > 6 ? true : false;
        ...
        closeLabelContainer.add(closeLabel), window.add(view), window.add(closeLabelContainer);

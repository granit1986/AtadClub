...
//Rare exceptions to indicate a bug in the _atadclub code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_atadclub = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_atadclub]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.atadclub.module"
